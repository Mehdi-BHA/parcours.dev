...
const CanvaWithIcon = (props) => {
    ...
    return (
        <g transform={`translate(${x},${y - iconTransform})`} x={x} y={y}>
            <g className="canva">
                <rect
                    x={0}
                    y={iconTransform}
                    ...
                    fill={color}
                    rx={5}
                    className="canva-rect"
                />
                {titleDivided.map((elem, k) => {
                    return (
                        <text
                            x={15}
                            y={baseHeight / 2 + k * spaceBetweenLines + iconTransform}
                    ...
                    return (
                        <text
                            ...
                            y={
                                ...
                                baseHeight / 2 +
                                spaceBetweenTitleAndSub +
                                (titleDivided.length + k) * spaceBetweenLines
                            }
                            dominantBaseline="middle"
                            textAnchor="start"
                ...
                {checked && <Check width={30} height={30} x={width-40} y={ iconTransform+10} />}
                {progress && (
                    <ProgressBar
                        progress={progress}
                        width={width}
                ...
                <Icon x={width*2 / 3} y={0} height={iconHeight} />
            </g>
        </g>
    );
};
...
const Icon = (props) => (
    <svg {...props} viewBox="0 0 256 361" preserveAspectRatio="xMinYMin">
        <g>
            <path
